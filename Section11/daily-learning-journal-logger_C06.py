'''
Challenge 06: Daily Learning Journal Logger
...
Example:
📅 2025-06-14 - 10:45 AM
Today I learned about how list comprerhensions work in Python!
...
now = datetime.datetime.now() # Pakai datetime.now untuk ambil waktu lengkap.
date_str = now.strftime("%Y-%m-%d - %I:%M %p") # Ini di-custom tampilan waktunya pakai strftime.

journal_entry = f"\n📅 {date_str}\n{entry}"

if rating: # Ini akan diisi kalau user isi rating, kalau tidak maka dianggap False dan tidak akan muncul dalam file.
    journal_entry += f"\nProductivity Rating: {rating}\n"
journal_entry += "\n" + "-" * 50

with open("learning_journal.txt", "a", encoding="utf-8") as f: # a itu append, jadi langsung lompat ke bagian terakhir dan sambung isinya. Isinya nambah tanpa bikin file baru (bukan kayak pakai w).

'''
Challenge 30: Store & Search Crypto Prices in SQLite

Goal:
- Save hourly top 10 crypto prices into a local SQLite database.
- Each record should include timestamp, coin ID, and price.
- Allow the user to search for a coin by name and return the lastest price.

Teaches: SQLite handling in Python, data storafe, search queries, API + DB integration.
'''
...
def main():
    ...
    if choice == "1":
        ...
        coin_name = input("Input coin name: ").strip().lower()
